          ...
          <div class="tile">
            <h3 class="tile-title">Son 10 Mesaj</h3>
				<?php
					...
					$sonmesaj->bindValue(":lim",(int) 3,PDO::PARAM_INT);
					$sonmesaj->execute();
							...
							<?php
								foreach($sonmesaj as $son){
									?>
										...
										<td><?php echo $son['eposta'];?></td>
										<td><?php echo $son['konu'];?></td>
										<td><?php echo $son['tarih'];?></td>
										<td><a href="<?php echo $yonetimUrl;?>/islemler.php?islem=mesajoku&id=<?php echo $son['id'];?>"><i class="fa fa-eye"></i></a></td>
										</tr>
									<?php
								}
							?>
        ...
        <div class="col-md-6">
          <div class="tile">
            <h3 class="tile-title">Son 10 Yorum</h3>
				<?php
					$sonyorum = $db->prepare("SELECT * FROM yorumlar INNER JOIN yazilar ON 
					yazilar.yazi_id = yorumlar.yorum_yazi_id WHERE yorum_durum=:d ORDER BY id DESC LIMIT :lim");
					$sonyorum->bindValue(":d",(int) 1,PDO::PARAM_INT);
							...
							<?php
								foreach($sonyorum as $son){
									?>
										...
										<td><?php echo $son['yorum_website'];?></td>
										<td><?php echo $son['yorum_tarih'];?></td>
										<td><a href="<?php echo $yonetimUrl;?>/islemler.php?islem=yorumoku&id=<?php echo $son['id'];?>"><i class="fa fa-eye"></i></a></td>
										</tr>
									<?php
								}
							?>
